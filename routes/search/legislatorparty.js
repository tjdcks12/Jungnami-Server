...
router.get('/:p_name/:l_name', async(req, res, next) => {
  var id; // 사용자 id
  ...
  try{
    ...
    select_legislator += "ON legislator.id = lv.lv_legislator_id WHERE legislator.l_party_name = ? AND legislator.name = ? ORDER BY score DESC";

    let result_legislator = await db.queryParamCnt_Arr(select_legislator,[req.params.p_name, req.params.l_name]);
