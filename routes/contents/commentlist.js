...
router.get('/:cc_contents_id', async(req, res) => {
	try{
		if(!(req.params.cc_contents_id)){
			res.status(403).send({
			...
			let commenttableInfo = await db.queryParamCnt_Arr(getcommentlistQuery, [req.params.cc_contents_id]);
